librg_network_add(&network_context, NETWORK_SEND_VOIP_DATA, [](librg_message* msg) {
    ...
    if(entity && entity->user_data && zpl_time_now() - send_time < 2.0f) {
        ...
        if (local_ped) {
            ...
            BASS_3DVECTOR my_rot = EXPAND_VEC(local_ped->GetInterface()->humanObject.entity.rotation);
            BASS_3DVECTOR my_pos = EXPAND_VEC(local_ped->GetInterface()->humanObject.entity.position);

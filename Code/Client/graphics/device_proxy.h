...
public:
	...
	{
		graphics::device_lost(this);
		HRESULT hr = p_Dev->Reset(presentParam);
		if (SUCCEEDED(hr))
		{
			graphics::device_reset(this);
		}
	...
	STDMETHOD(LightEnable)(DWORD index,int enable)
	{
		return p_Dev->LightEnable(index,enable);
	}

	STDMETHOD(GetLightEnable)(DWORD index,int *pEnable)
	{

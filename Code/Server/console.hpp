...
inline void console_draw(const char* format, ...);
inline void console_printf(const char* format, ...);
inline char console_update_loader();

#ifdef _WIN32 
inline void console_init_w32() {
...
inline void console_draw_unix(const char* format, va_list va) {
...
#ifdef _WIN32
    console_draw_w32(format, arglist);
#else
    console_draw_unix(format, arglist);
#endif
    va_end(arglist);
}

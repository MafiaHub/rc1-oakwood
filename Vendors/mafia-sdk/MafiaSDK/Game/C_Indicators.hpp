...
namespace MafiaSDK
{
	...
	namespace C_Indicators_Hooks
	{
		...
		{
			std::function<void()> afterDrawAll;
		};
		
		namespace Functions
		{
			inline void OnAfterAllDraw() 
			{
				if(FunctionsPointers::afterDrawAll != nullptr) 
					FunctionsPointers::afterDrawAll();
			}
		};
		...
		inline void HookAfterDrawAll(std::function<void()> funcitonPointer)
		{
			FunctionsPointers::afterDrawAll = funcitonPointer;

			MemoryPatcher::InstallJmpHook(0x005BD905, (unsigned long)&NakedFunctions::AfterDrawAll_Hook);
		}
#endif
	};
